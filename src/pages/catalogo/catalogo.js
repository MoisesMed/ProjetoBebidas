...
class catalogo extends React.Component {
    ...
    obterObjeto(id, descricao, detalhe, resumo, preco, imagem){
        const objeto = {
            'id': id,
            'descricao': descricao,
            'detalhe': detalhe,
    ...
    obterItemLista(item, categoria) {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate('produto', {item : item, categoria: categoria})}>
    ...
    async obterCatalogo() {
        ...
        await firestore().collection("produto").get()
            .then(querySnapshot => {
                querySnapshot.docs.forEach(doc => {
                    objeto = this.obterObjeto(doc.id, doc.data().descricao, doc.data().detalhe, doc.data().resumo, doc.data().preco, doc.data().imagem);
                    switch (doc.data().categoria.id) {                        
                        case this.state.ID_DESTILADO:
                            arrDestilados.push(objeto);
                            break;
                        case this.state.ID_VINHO:
                            arrVinho.push(objeto);
                            break;
                        case this.state.ID_CERVEJA:
                            arrCerveja.push(objeto);
                            break;
                        case this.state.ID_PETISCOS:
                            arrPetisco.push(objeto);
                            break;
                        case this.state.ID_SEM_ALCOOL:
                            arrSemAlcool.push(objeto);
                            break;
                        default:
                            break;
                    }
        ...
        this.setState({ arrayCerveja: arrCerveja });
        this.setState({ arrayPetisco: arrPetisco });
        this.setState({ arraySemAlcool: arrSemAlcool });
        this.setState({ arrayVinho: arrVinho });
    }
            ...
            <ScrollView>
                <View style={styles.container}>
                    ...
                    <FlatList
                        ...
                        data={this.state.arrayCerveja}
                        horizontal={true}
                        renderItem={({ item }) => this.obterItemLista(item, 'Cervejas')}/>                    

                    <Text style={styles.title}>Destilados</Text>
                    <FlatList
                        style={styles.divisoria}
                        data={this.state.arrayDestilados}
                        horizontal={true}
                        renderItem={({ item }) => this.obterItemLista(item, 'Destilados')}/>

                    <Text style={styles.title}>Vinhos</Text>
                    <FlatList
                        style={styles.divisoria}
                        data={this.state.arrayVinho}
                        horizontal={true}
                        renderItem={({ item }) => this.obterItemLista(item, 'Vinhos')}/>

                    <Text style={styles.title}>Sem Álcool</Text>
                    <FlatList
                        style={styles.divisoria}
                        data={this.state.arraySemAlcool}
                        horizontal={true}
                        renderItem={({ item }) => this.obterItemLista(item, 'Sem Álcool')}/>

                    <Text style={styles.title}>Petiscos</Text>
                    <FlatList
                        style={styles.divisoria}
                        data={this.state.arrayPetisco}
                        horizontal={true}
                        renderItem={({ item }) => this.obterItemLista(item, 'Petiscos')}/>
